...
export const PLATFORMS = {
  ...
  wenxin: {
    ...
    replySelectors: [
      '.last-answer-box .answer-container',
...
export function runPlaywright(
  code,
  { session = Number(process.env.PW_SESSION) || 1, timeout = 30_000 } = {},
) {
...
export function clickButton(selector, options = {}) {
  return runPlaywright(`await state.page.click('${selector}'); return 'clicked';`, options)
}
 ...
 */
export function waitForResponse(options = {}) {
  const {
    stopSelector = 'button:has-text("停止"), button:has-text("Stop")',
    pollInterval = 2000,
    maxWait = 120_000,
