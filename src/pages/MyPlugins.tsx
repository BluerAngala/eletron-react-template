        ...
        <div className="space-y-3">
          {sorted.map((plugin) => {
            ...
            return (
              <div
                key={plugin.path}
                className="group flex items-center gap-4 rounded-3xl border border-border-default bg-surface/90 p-4 shadow-[0_12px_24px_-28px_rgba(15,23,42,0.5)] backdrop-blur transition-all hover:border-accent/40"
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-0.5 truncate text-xs text-foreground-secondary">
                    {plugin.description ||
                      (plugin.author ? `${t('myplugins.author')}: ${plugin.author}` : plugin.name)}
                  </p>
                </div>
