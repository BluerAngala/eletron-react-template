...
export function Logs() {
  ...
  useEffect(() => {
    ...
    window.logAPI?.read().then((history) => {
      if (cancelled) return
      const liveEntries = [...entriesRef.current, ...pendingRef.current]
      const historyEntries = new Set(history.map(logFingerprint))
      const merged = [
        ...history,
        ...liveEntries.filter((entry) => !historyEntries.has(logFingerprint(entry))),
      ]
      setEntries(merged)
  ...
  const handleCopyEntry = useCallback(async (entry: LogEntry) => {
    ...
    try {
      ...
      setTimeout(() => setCopiedEntry((current) => (current === entry ? null : current)), 1500)
    } catch {
      log.error('copy-failed', {})
    }
  }, [])
...
function logFingerprint(entry: LogEntry): string {
  return `${entry.ts}:${entry.level}:${entry.scope}:${entry.message}:${JSON.stringify(entry.data)}`
}
