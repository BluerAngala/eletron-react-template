...
export class PluginWindowAPI {
  ...
  private setupIPC(): void {
    ipcMain.on('createBrowserWindow', (event, args: { url: string; options?: Electron.BrowserWindowConstructorOptions }) => {
    ...
    ipcMain.on('pluginBrowserWindowMethod', (event, args: { id: number; method: string; args?: unknown[] }) => {
      const { id, method, args: methodArgs = [] } = args || {}
      const win = this.createdWindows.get(id)
      if (!win) { event.returnValue = { success: false, error: '窗口不存在' }; return }
      try {
    ...
    ipcMain.handle('pluginBrowserWindowInvoke', async (_event, args: { id: number; method: string; args?: unknown[] }) => {
      const { id, method, args: methodArgs = [] } = args || {}
      const win = this.createdWindows.get(id)
    ...
    ipcMain.on('ipc-send-to', (event, webContentsId: number, channel: string, ...args: unknown[]) => {
