 ...
 *
 * 用法:
 *   pnpm icons                  # 默认源 resources/assets/logo.svg
 *   pnpm icons --input foo.svg  # 指定源文件
 *
 * 输出:
 *   build/icon.png          1024×1024 打包源图，electron-builder 自动派生 .icns / .ico
 *   build/icon.ico          多尺寸 Windows 图标
 *   resources/public/favicon.ico      多尺寸 favicon（浏览器 / 开发态 / 窗口图标）
 *   resources/public/logo-<size>.png  16/24/32/48/64/128/256/512 各尺寸（页面、托盘等使用）
 *
...
const INPUT =
  inputIndex !== -1
    ...
    : path.join(ROOT, 'resources/assets/logo.svg')
...
async function main() {
  ...
  // 各尺寸 PNG → resources/public/logo-<size>.png
